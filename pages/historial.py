...
# Importaciones para la re-generación del PDF en el historial
...
# ==================================================
# CALLBACK DE FORMATEO INTELIGENTE (RUT O NOMBRE)
# ==================================================
def formatear_busqueda_callback():
    busqueda_actual = st.session_state.busqueda_input
...
# ==================================================
# FUNCIÓN RE-GENERADORA DEL PDF DESDE EL HISTORIAL
# ==================================================
def generar_pdf_desde_historial(estudio, paciente, mediciones_estudio):
    """Genera el reporte PDF con la misma estética GE Healthcare usando datos históricos."""
    buffer = io.BytesIO()
    ...
    style_h1 = ParagraphStyle('Header1', fontName='Helvetica-Bold', fontSize=14, leading=16, textColor=colors.HexColor("#1A365D"))
    ...
    style_table_header = ParagraphStyle('TableHeader', fontName='Helvetica-Bold', fontSize=9, leading=11, textColor=colors.white)
    ...
    # Extraer la hora de creación de forma segura
    ...
    data_paciente = [
        [Paragraph("<b>Paciente:</b>", style_body), Paragraph(nombre_p, style_body), Paragraph("<b>Fecha Estudio:</b>", style_body), Paragraph(fecha_str, style_body)],
        [Paragraph("<b>ID / RUT:</b>", style_body), Paragraph(paciente.rut, style_body), Paragraph("<b>Edad:</b>", style_body), Paragraph(f"{edad_p} años", style_body)],
        [Paragraph("<b>Género:</b>", style_body), Paragraph(paciente.sexo or "—", style_body), Paragraph("<b>Médico:</b>", style_body), Paragraph(estudio.medico or "—", style_body)],
        [Paragraph("<b>Tipo Estudio:</b>", style_body), Paragraph(estudio.tipo_estudio, style_body), Paragraph("<b>Motivo:</b>", style_body), Paragraph(estudio.motivo or "—", style_body)]
    ]
    ...
    # Tabla de Mediciones registradas en este estudio específico
    story.append(Paragraph("<b>MEDICIONES ECOCARDIOGRÁFICAS</b>", style_body_bold))
    story.append(Spacer(1, 6))
    
    if mediciones_estudio:
        data_mediciones = [[Paragraph("Variable / Parámetro", style_table_header), Paragraph("Valor", style_table_header), Paragraph("Unidad", style_table_header)]]
        
        for m, v in mediciones_estudio:
            valor_final = str(m.valor_num) if m.valor_num is not None else (m.valor_texto or "")
            unidad_var = v.unidad if (v and v.unidad) else "—"
            ...
            data_mediciones.append([
                ...
                Paragraph(unidad_var if unidad_var else "—", style_body)
            ])
        ...
        story.append(Paragraph("<i>No se encontraron mediciones asociadas a este registro.</i>", style_body))
        
    story.append(Spacer(1, 15))
    
    # Bloque Narrativo Guardado
    ...
    story.append(KeepTogether(informe_narrativo))
    doc.build(story)
    buffer.seek(0)
    return buffer
...
try:
    ...
    variables = session.query(Variable.codigo, Variable.nombre, Variable.unidad).order_by(Variable.nombre).all()
    opciones_vars = {f"{v.nombre} ({v.codigo})": v.codigo for v in variables}
    ...
    # ==================================================
    # 3. LISTA DE PACIENTES (ALFABÉTICO CON BÚSQUEDA)
    # ==================================================
    ...
    # Crear las dos columnas con pesos iguales (50% y 50%)
    ...
    # El filtro de la máscara se ejecuta fuera/entre las columnas para actualizar el DataFrame
    ...
    # Dibujar el selector o la advertencia en la columna derecha
    with col_seleccion:
        if not df_pacientes.empty:
            paciente_seleccionado_rut = st.selectbox(
                "✅ Seleccione un paciente para ver su historial",
                options=df_pacientes["RUT"].tolist(),
                index=None,                  # No selecciona a nadie por defecto
                placeholder="Seleccionar 👇", # Texto por defecto agradable
                format_func=lambda rut: f"{df_pacientes[df_pacientes['RUT'] == rut]['Nombre completo'].iloc[0]} ({rut})"
            )
            
            # Traer la instancia completa del Paciente si el usuario ya seleccionó uno
            if paciente_seleccionado_rut:
                paciente_obj = session.query(Paciente).filter(Paciente.rut == paciente_seleccionado_rut).first()
            else:
                paciente_obj = None
        else:
            st.markdown("<br>", unsafe_allow_html=True) # Pequeño espacio para alinear con el input
            st.warning("No se encontraron pacientes.")
            paciente_seleccionado_rut = None
            paciente_obj = None

    with st.expander("Ver pacientes en la base de datos"):
        st.dataframe(df_pacientes, use_container_width=True)

    # ==================================================
    # 4. HISTORIAL DETALLADO (INFORMES + DESCARGA PDF)
    # ==================================================
    if paciente_seleccionado_rut:
        st.divider()
        nombre_completo_paciente = df_pacientes[df_pacientes['RUT'] == paciente_seleccionado_rut]['Nombre completo'].iloc[0]
        st.subheader(f"📈 Historial Clínico de {nombre_completo_paciente}")

        # Mostrar la fecha y hora de registro del paciente (si existe en base de datos)
        if hasattr(paciente_obj, 'fecha_creacion') and paciente_obj.fecha_creacion:
            st.caption(f"🕒 *Paciente registrado en el sistema el: {paciente_obj.fecha_creacion.strftime('%d/%m/%Y a las %H:%M:%S')}*")

        estudios_paciente = session.query(Estudio).filter(
            Estudio.paciente_rut == paciente_seleccionado_rut,
            Estudio.fecha_estudio >= fecha_desde,
            Estudio.fecha_estudio <= fecha_hasta
        )
        if tipo_seleccionado:
        ...
        if not estudios_paciente:
            ...
            opciones_estudios = {
                f"📅 {est.fecha_estudio.strftime('%d/%m/%Y')} — {est.tipo_estudio} (Dr/a. {est.medico or 'No registrado'})": est 
                for est in estudios_paciente
            }
            
            estudio_seleccionado_obj = st.selectbox(
                "Selecciona un examen del historial para desplegar el informe guardado o descargar su versión PDF:",
            ...
            if estudio_seleccionado_obj:
                ...
                mediciones_este_estudio = session.query(Medicion, Variable)\
                    .outerjoin(Variable, Medicion.codigo_variable == Variable.codigo)\
                    .filter(Medicion.estudio_id == est_objeto.id)\
                    .all()
                
                # Extracción segura de la hora de registro del estudio
                hora_registro_estudio = est_objeto.fecha_creacion.strftime('%H:%M:%S') if hasattr(est_objeto, 'fecha_creacion') and est_objeto.fecha_creacion else "No registrada"

                # Tarjeta contenedora del informe
                with st.container(border=True):
                    ...
                    if est_objeto.observaciones:
                        ...
                        st.write("*No se registraron observaciones narrativas ni conclusiones escritas en este informe.*")
                    
                    pdf_historial = generar_pdf_desde_historial(est_objeto, paciente_obj, mediciones_este_estudio)
                    st.download_button(
                        label="📥 Re-generar y Descargar PDF de este Examen",
                        data=pdf_historial,
                        file_name=f"Informe_{est_objeto.tipo_estudio.replace(' ', '_')}_{paciente_obj.rut}_{est_objeto.fecha_estudio.strftime('%Y%m%d')}.pdf",
            ...
            # --------------------------------------------------
            # PROCESAMIENTO GENERAL DE GRÁFICOS
            # --------------------------------------------------
            mediciones = []
            for est in estudios_paciente:
            ...
            if not df_med.empty:
                ...
                tiene_hora = (df_med["fecha_dt"].dt.hour != 0).any() or (df_med["fecha_dt"].dt.minute != 0).any()
                df_med["fecha_str"] = df_med["fecha_dt"].dt.strftime("%d/%m/%Y %H:%M" if tiene_hora else "%d/%m/%Y")
                ...
                vars_a_graficar = st.multiselect(
                    "Seleccione las variables ecocardiográficas para trazar sus curvas de evolución:",
                    options=vars_disponibles,
                    default=[v for v in vars_disponibles if v in ["FEVI", "AVAo EC", "Gradiente medio Ao"]][:3]
                )

                if vars_a_graficar:
                    df_num = df_med[df_med["variable"].isin(vars_a_graficar) & df_med["valor_num"].notna()].copy()
                    if not df_num.empty:
                        fig = px.line(
                            df_num,
                            x="fecha_dt",
                            ...
                            labels={"fecha_dt": "Fecha del Examen", "valor_num": "Valor Obtenido", "variable": "Parámetro"}
                        )
    ...
    mediciones_globales = session.query(
        Medicion.valor_num,
        Estudio.fecha_estudio,
        Paciente.nombres,
     ...
     .order_by(Estudio.fecha_estudio).all()

    if mediciones_globales:
        df_global = pd.DataFrame(mediciones_globales, columns=["valor", "fecha", "nombres", "apellido_paterno", "apellido_materno", "rut"])
